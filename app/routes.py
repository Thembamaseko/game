...
def init_routes(app):
    ...
    def index():
        ...
        if request.method == 'POST':
            ...
            if (ballance < int(amount)):
                print("Insufficient coins to place bet")
    ...
    def profile():
        ...
        for bet in past_bets:
            print(f"Bet ID: {bet.id}, Choice: {bet.choice}, Amount: {bet.amount}, Spin ID: {bet.spin_id}")
            spin = Spin.query.get(bet.spin_id)
            if spin:
                print(f"Associated Spin Result: {spin.result}")
            else:
                print("No associated spin found.")
        
        return render_template('profile.html', user=current_user, bets= past_bets,spin=spin)
